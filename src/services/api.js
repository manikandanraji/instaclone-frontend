...
export const getFeed = async () => {
  ...
  return await api.get("/users/feed", {
    headers: { Authorization: `Bearer ${token}` },
  });
};
...
export const addComment = async ({ postId, body }) => {
  const { token } = JSON.parse(localStorage.getItem("user"));
  return await api.post(`/posts/${postId}/comments`, body, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
...
export const toggleSave = async ({ postId }) => {
  const { token } = JSON.parse(localStorage.getItem("user"));
  return await api.get(`/posts/${postId}/togglesave`, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
...
export const toggleLike = async ({ postId }) => {
  const { token } = JSON.parse(localStorage.getItem("user"));
  return await api.get(`/posts/${postId}/togglelike`, {
    headers: { Authorization: `Bearer ${token}` },
  });
};
